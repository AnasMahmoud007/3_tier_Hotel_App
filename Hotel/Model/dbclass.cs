...
namespace Hotel.Model
{
    public class dbclass
    {
        ...
        public DataTable ShowTable(string Tname)
        {
            ...
            {
                // Rethrow the exception to expose the actual database error
                throw new Exception("Error executing ShowTable for " + Tname + ": " + ex.Message, ex);
            }
            finally
            {
                con.Close();
        ...
        public DataRow GetUser(string tableName, string username, string password)
        {
            DataTable dt = new DataTable();
            string usernameColumn = (tableName == "Staff") ? "SUsername" : "UserName";
            ...
            {
                // Rethrow the exception to expose the actual database error
                throw new Exception($"Error executing GetUser for {tableName}: " + ex.Message, ex);
            }
            finally
            {
                con.Close();
        ...
        public (DataRow userRow, string role) AuthenticateUserAndGetRole(string username, string password)
        {
            // Check Guest table
            DataTable dtGuest = new DataTable();
            string queryGuest = "SELECT * FROM Guest WHERE UserName = @Username AND Password = @Password";
            using (SqlCommand cmdGuest = new SqlCommand(queryGuest, con))
            {
                cmdGuest.Parameters.AddWithValue("@Username", username);
                cmdGuest.Parameters.AddWithValue("@Password", password);
                con.Open();
                dtGuest.Load(cmdGuest.ExecuteReader());
                con.Close();
                if (dtGuest.Rows.Count > 0)
                {
                    return (dtGuest.Rows[0], "Guest");
                }
            }
            ...
            using (SqlCommand cmdAdmin = new SqlCommand(queryAdmin, con))
            {
                ...
                con.Open();
                dtAdmin.Load(cmdAdmin.ExecuteReader());
                con.Close();
                if (dtAdmin.Rows.Count > 0)
                {
                    return (dtAdmin.Rows[0], "Admin");
                }
            }

            // Check Manager table
            DataTable dtManager = new DataTable();
            string queryManager = "SELECT * FROM Manager WHERE UserName = @Username AND Password = @Password";
            using (SqlCommand cmdManager = new SqlCommand(queryManager, con))
            {
                cmdManager.Parameters.AddWithValue("@Username", username);
                cmdManager.Parameters.AddWithValue("@Password", password);
                con.Open();
                dtManager.Load(cmdManager.ExecuteReader());
                con.Close();
                if (dtManager.Rows.Count > 0)
                {
                    return (dtManager.Rows[0], "Manager");
                }
            }
        ...
        /*-------------------------Insert Table Query----------------------*/
        public string SignUp(string userName, string password, string name, string userInformation)
        {
            string cr = string.Empty;
            
            string query = "INSERT INTO Guest (UserName, Password, Name, UserInformation) VALUES (@UserName, @Password, @Name, @UserInformation);";
            try
            {
                con.Open();
                SqlCommand cmd = new SqlCommand(query, con);
            ...
            {
                // Rethrow the exception to expose the actual database error
                throw new Exception("Error during SignUp for user " + userName + ": " + ex.Message, ex);
            }
            finally
            {
                con.Close();
        ...
        public string InsertReservation(int RoomNumber, string UserName, DateTime CheckInDate, DateTime CheckOutDate)
        {
            string cr = string.Empty;
            string query = "INSERT INTO Reservation (RoomNumber, UserName, CheckInDate, CheckOutDate) VALUES (@RoomNumber, @UserName, @CheckInDate, @CheckOutDate);";
            try
            {
                con.Open();
                SqlCommand cmd = new SqlCommand(query, con);
            ...
            {
                // Rethrow the exception to expose the actual database error
                throw new Exception("Error inserting reservation for user " + UserName + ": " + ex.Message, ex);
            }
            finally
            {
                con.Close();
        ...
        public string InsertFeedback(string UserName, DateTime FeedbackDate, string Comments, int Rating)
        {
            string cr = string.Empty;
            string query = "INSERT INTO Feedback (UserName, FeedbackDate, Comments, Rating) VALUES (@UserName, @FeedbackDate, @Comments, @Rating);";
            try
            {
                con.Open();
                SqlCommand cmd = new SqlCommand(query, con);
        ...
        public string DeleteStaff(string SUsername)
        {
            string cr = string.Empty;
            string query = "DELETE FROM Staff WHERE SUsername = @SUsername;";
            try
            {
                con.Open();
                SqlCommand cmd = new SqlCommand(query, con);
        ...
        public string UpdateServices(int ServiceID, string AmenityName, string Description, int Availability, decimal AdditionalCharges, string icon)
        {
            string cr = string.Empty;
            string query = "UPDATE Services SET AmenityName = @AmenityName, Description = @Description, Availability = @Availability, AdditionalCharges = @AdditionalCharges, icon = @icon WHERE ServiceID = @ServiceID;";
            try
            {
                con.Open();
                SqlCommand cmd = new SqlCommand(query, con);
